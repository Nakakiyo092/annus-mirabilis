...
'Y' |   YES+  |   Yn[CR]               | Setup with CANFD data bit rate where n is 0-5.
...
Setup CANFD nominal bit rate, where `n` is 0-5.
...
Setup CANFD nominal bit rate to 125kbps.
...
Setup CANFD nominal bit-rates and bit-timing, where `dd`, `xx`, `yy` and `zz` are hex values.
...
Setup CANFD nominal bit rate to 125kbps and sampling point to 88.33%.
...
Setup CANFD data bit-rates, where `n` is 0-5.
...
Setup CANFD data bit rate and bit timing, where `dd`, `xx`, `yy` and `zz` are hex values.
...
Setup CANFD data bit rate to 2Mbps and sampling point to 73.33%.
This is the default setting when the device is powered on.
...
Open the CANFD channel in normal mode.
...
Open the channel, TX LED is turned OFF.
...
Open the CANFD channel in listen only mode.
...
Close the channel, TX LED is turned ON.
...
For example, `F84[CR]` is responded when the device has detected bus error and the error counter is warning level.
...
Set Acceptance Mask to 0xFFFFFFFF, which makes pass all filter i.e. all frames are received.
This is default when power on.


## V[CR]

Get version number of both hardware and software
...
- `Z0`  Timestamp off
- `Z1`  Milli second timestamp (Reset to 0 at 0xEA60 ms = 60,000 ms)
- `Z2`  Micro second timestamp (Reset to 0 at 0xD693A400 us = 3600,000,000 us)
...
Turn on the micro second time stamp feature (default).
...
Turn off the auto startup feature (default).
